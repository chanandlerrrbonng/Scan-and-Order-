...
export default function Catalog({ searchQuery, activeCategory, sortOption, onSortChange }) {
  ...
  // Mount-time fetch + periodic refresh so availability changes made by the
  // admin appear for customers without a manual page reload.
  useEffect(() => {
    const controller = new AbortController();
    let cancelled = false;

    function loadMenu() {
      setStatus((s) => (s === 'ready' ? s : 'loading')); // don't flash skeleton on refresh
    ...
    const id = setInterval(loadMenu, 30000); // refresh every 30s
